...
io.on('connection', (socket) => {
    ...
    // socket.broadcast.emit('message', generateMessage('A new user has joined the chat box')) // emit to all clients except this socket

    socket.on('join', (/*{ username, room }*/ options, callback) => {
        const { error, user} = addUser({  id: socket.id, /*username, room*/ ...options  })
